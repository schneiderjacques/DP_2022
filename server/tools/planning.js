...
/**
 * Méthode retournant tous les rendez-vous d'un utilisateur à une année donnée
 * @param req
 * Requête
 * @param res
 * Réponse
 */
function year_view(req, res) {
  ...
  let planning = get_planning(req);
  if (planning) {
    // Récupérer les rendez-vous de l'année
    let rdvs = [];
    for (let i = 0; i < planning.length; i++) {
      let date = new Date(planning[i].date);
      if (date.getFullYear() == req.params.year) {
        rdvs.push(planning[i]);
      }
    }
    res.set({
      ...
      for (var k = 0; k < users[i].token.length; k++) {
        if (users[i].token[k] == req.headers.authorization.substring(7)) {
          ...
          for (var j = 0; j < planning.length; j++) {
            ...
            if (new_date_debut == date_debut && new_date_fin == date_fin) {
              res.sendStatus(409);
              return;
            }
          }
