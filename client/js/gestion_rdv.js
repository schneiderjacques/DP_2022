...
function createRdv(){
    ...
    if(day.length === 0 ||  (/^(0?[1-9]|[12][0-9]|3[01])\/(0?[1-9]|1[012])\/\d{4}$/.test(day)) ) {
        console.log('erreur')
        alert.children[2].innerHTML = 'Veuillez entrer une date valide';
        showAlert(true);
        return;
    }
    if(debutRdv.length === 0 || (/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(debutRdv[0]))) {
        alert.children[2].innerHTML = 'Veuillez entrer une heure de début valide';
        showAlert(true);
        return;
    }
    if(finRdv.length === 0 || (/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(finRdv[0]))) {
    ...
    fetchDataConnected(JSON.stringify(data), 'POST', 'add_appointment').then((response) => {
        refreshAllCalendars();
    });
...
function showAlert(state){ //Affiche l'alerte dans le modal d'ajout de rdv
    var alert = document.getElementById('alert-3');
...
const colors=["#B91C1C", "#FFA500", "#FFFF00", "#008000", "#1D4ED8", "#4B0082", "#EE82EE"];
var selectedColor = 0; 
initColors();
...
function initColors(){
    ...
    for (let i = 0; i < colors.length; i++) {
        ...
        label.classList.add("ml-2", "relative", "p-0.5","rounded-full", "flex", "items-center", "justify-center", "cursor-pointer", "focus:outline-none");
        if(i == selectedColor){
        ...
        span.classList.add("h-8", "w-8", "bg-" + colors[i], "border", "border-black", "border-opacity-10", "rounded-full");
