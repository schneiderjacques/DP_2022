...
import { getAllDaysFromMonth, setPreviousDay, searchDateInArray } from './date_tools.js'
...
document.getElementById('btn-next-month').addEventListener('click', function () { //Bouton pour afficher le mois suivant
    ...
    refreshCalendarMonth(currentMonth, currentYear);
...
export function refreshCalendarMonth(month, year){
...
function printCalendarByMonth(days, month) { //Affiche le calendrier
    ...
    for (let i = 0; i < days.length; i++) {
        ...
        if (dt.getDate() == new Date().getDate() && dt.getMonth() == new Date().getMonth() && dt.getFullYear() == new Date().getFullYear()) { //Regarde si c'est la date du jour on rajoute une classe
