import { setPreviousDay, setNextDay, formatNumber, getDayName, getFirstDayOfWeek, getAllDaysFromWeek, initHours, getMinDiff, timeToDecimal, searchDateInArray, getMonthName, refreshAllCalendars, convertHex } from './date_tools.js'
...
function setUpEventsByDate(date, eventsArray) { //Met en place les événements dans le tableau
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        createWeekRdv(dayColumnIndex, dayDebut, events[i].nom, gridRow, span, events[i].couleur);
...
function createWeekRdv(dayColumnIndex, dayDebut, event_name, gridRow, span, color) {
    ...
    a.classList.add("group", "absolute", "inset-1", "flex", "flex-col", "cursor-pointer", "overflow-y-auto", "rounded-lg", "p-2", "text-xs", "leading-5");
