      ...
      <div style="width: 165%" class="flex max-w-full flex-none flex-col sm:max-w-none md:max-w-full">
        <div ref="containerNav"
          ...
          <div
            ...
            <div class="flex items-center justify-center py-3" v-for="day in days" v-bind:key="day">
              <span :class="areDateEqual(day, new Date()) ? 'flex items-baseline' : ''"> {{ getDayName(day).substring(0,3) }}
        ...
        <div class="flex flex-auto">
          ...
          <div class="grid flex-auto grid-cols-1 grid-rows-1">
            ...
            <ol class="col-start-1 col-end-2 row-start-1 grid grid-cols-1 sm:grid-cols-7 sm:pr-8"
              ...
              <RdvWeekVue v-for="rdv in rdvs" v-bind:key="rdv" :rdv="rdv"></RdvWeekVue>
            </ol>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    getClass(day) {
      ...
      if (areDateEqual(day, this.date)) { //Si la date étudiée est celle du jour
    ...
    getRdv() {
      ...
      const dateFormat = firstDayOfWeek.getFullYear() + "-" + formatNumber(firstDayOfWeek.getMonth() + 1) + "-" + formatNumber(firstDayOfWeek.getDate())
      fetchDataGet(
          "week_planning/" + dateFormat,
      ).then((res) => {
        this.rdvs = res;
      }).catch((err) => {
        console.log(err);
