    ...
    <div class="isolate flex flex-auto overflow-hidden bg-white">
      <div ref="container" class="flex flex-auto flex-col overflow-auto">
        <div ref="containerNav"
          ...
          <button type="button" class="flex flex-col items-center pt-3 pb-1.5" v-for="day in days" v-bind:key="day">
        ...
        <div class="flex w-full flex-auto">
          ...
          <div class="grid flex-auto grid-cols-1 grid-rows-1">
            ...
            <ol class="col-start-1 col-end-2 row-start-1 grid grid-cols-1"
                ...
                <RdvDayVue v-for="rdv in rdvs" v-bind:key="rdv.id" :rdv="rdv"/>
...
import {formatNumber, getAllDaysOfWeek, getDayName, areDateEqual} from "@/js/date_tools";
...
export default {
  ...
  methods: {
    getDayName,
    getRdv() {
      const dateFormat = this.date.getFullYear() + "-" + formatNumber(this.date.getMonth() + 1) + "-" + formatNumber(this.date.getDate())
      fetchDataGet(
          "day_planning/" + dateFormat,
      ).then((res) => {
        this.rdvs = res;
      }).catch((err) => {
        console.log(err);
